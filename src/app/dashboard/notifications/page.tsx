...
export default function NotificationsPage() {
  ...
  const tabs = ["All", "Financial", "Security", "Management", "System"];
  ...
  return (
    <div className="bg-white rounded-[24px] p-6 border border-border/40 shadow-sm w-full max-w-[1000px] min-h-[800px] mx-auto flex flex-col animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className="h-10 px-4 rounded-xl border-border/30 text-[11px] font-bold text-slate hover:bg-surface transition-all gap-2"
          >
          ...
          <Button
            ...
            className="h-10 px-4 rounded-xl border-border/30 text-[11px] font-bold text-red-500 hover:bg-red-50 transition-all gap-2"
          >
      ...
      <div className="flex-1 space-y-2.5">
        {filteredNotifications.length > 0 ? (
          filteredNotifications.map((n) => (
            <div
              key={n.id}
              className={`group relative p-4 bg-white border rounded-[20px] flex items-start gap-4 transition-all duration-300 hover:border-border/60 ${
                n.status === "unread"
            ...
            >
              ...
              <div className="flex-1 pr-8">
                <div className="flex items-center gap-3 mb-1">
                  <h3
                    className={`text-[14px] font-bold font-outfit tracking-tight ${n.status === "unread" ? "text-dark" : "text-slate/70"}`}
                  >
                    {n.title}
                  </h3>
                  <span className="text-[9px] font-bold text-slate/30 uppercase tracking-tighter bg-surface/50 px-2 py-0.5 rounded-md">
                    {n.type}
                  </span>
                  {n.status === "unread" && (
                ...
                <p className="text-[12px] font-medium text-slate/50 leading-relaxed max-w-[650px]">
                  {n.description}
                </p>
                <div className="flex items-center gap-3 mt-2.5">
                  <div className="flex items-center gap-1 text-[9px] font-bold text-slate/30 uppercase tracking-tight">
                    <Calendar className="h-2.5 w-2.5" />
                    {n.date}
                  </div>
                  <div className="h-1 w-1 rounded-full bg-border/40" />
                  <div className="text-[9px] font-bold text-primary/60 uppercase">
                    {n.time}
                  </div>
                </div>
              </div>
      ...
      {notifications.length > 0 && (
        <div className="mt-8 pt-6 border-t border-border/10 flex items-center justify-center">
