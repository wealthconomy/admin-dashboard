...
function InterestEarnedCell({ savingType, interestAmount, wealthPactAmount }: { savingType: SavingType; interestAmount?: number; wealthPactAmount?: number }) {
  const total = (interestAmount || 0) + (wealthPactAmount || 0);
...
function GroupCard({ group }: { group: WealthGroupData }) {
  ...
  const totalWealthpact = group.members.reduce((s, m) => s + (m.wealthPactAmount ?? 0), 0);
...
export default function PlanUsersPage() {
  ...
  const summaryCards = isWealthGroup ? [
    { title: "Total Groups", value: groups.length.toLocaleString(), subtext: "All active groups", icon: Users, color: "bg-[#E6F9F9] text-[#155D5F]", dotColor: "bg-[#155D5F]" },
    ...
    { title: isWealthFix ? "Avg Term" : isWealthGoal ? "Goal Target" : "Avg Balance", value: isWealthFix ? "12 months" : isWealthGoal ? formatCurrency(totalGoalTarget) : formatCurrency(totalBalance / Math.max(1, totalMembers)), subtext: isWealthFix ? "lock-in period" : isWealthGoal ? "combined targets" : "per member", icon: Target, color: "bg-[#E6F9F9] text-[#155D5F]", dotColor: "bg-[#155D5F]" },
